...
#if 1//dennis
extern uint8_t wifi_ssid_auth[20];
extern uint8_t wifi_password_auth[20];
extern uint8_t wifi_security_auth[20];

extern uint8_t wifi_ssid_auth_len;
extern uint8_t wifi_password_auth_len;
extern uint8_t wifi_security_auth_len;
...
static int wifi_connect(void)
{
    ...
    connect_param.ap_credentials.security = WIFI_SECURITY;
	 ...
	 /* Configure the connection parameters for the Wi-Fi interface. */
    memset(&connect_param, 0, sizeof(cy_wcm_connect_params_t));
    memcpy(connect_param.ap_credentials.SSID, wifi_ssid_auth, wifi_ssid_auth_len);
    memcpy(connect_param.ap_credentials.password, wifi_password_auth, wifi_password_auth_len);
	if(wifi_security_auth[0]==0x31)
	{		
    ...
    for (retry_count = 0; retry_count < 3/*MAX_WIFI_CONN_RETRIES*/; retry_count++)
    {
