...
void cybt_platform_set_next_timeout(uint64_t abs_tick_us_to_expire)
{
	...
	#if 0//dennis
	cy_log_msg(CYLF_AUDIO, CY_LOG_WARNING, "%s(), time_to_expire_in_us=%ld\n", __func__,time_to_expire_in_us);
	#endif
                                                 ...
                                                 )
{
    ...
    if (length < 207)
    {
        ...
        {
            printf ("0x%02x ", p_data[ii]);
        }
        printf ("\n");
    }
    else
    {
        printf("download firmware length = %d\n", length);
    }
	#endif
                                               ...
                                               )
{
	...
	#if 0//dennis
    printf ("read:\n");
    for (uint32_t ii = 0; ii < *p_length; ++ii )
    {
        printf ("0x%02x ", p_data[ii]);
    }
    printf ("\n");
	...
	if((p_data[0]==0x1b)&&(p_data[1]==0x04)&&(p_data[2]==0x02)) 
	{
		printf ("DBFW event 00000000000000000000000000000000000\n");
	}
...
void cybt_idle_timer_cback(cy_timer_callback_arg_t arg)
{
	...
	printf("88\n");
	printf("test_cnt=%d\n",test_cnt);
	//cy_log_msg(CYLF_AUDIO, CY_LOG_WARNING, "%s()\n", __func__);
	#endif
    cybt_platform_disable_irq();
    ...
    while(1)
    {
        ...
        switch(action)
        {
            case SLEEP_ACT_START_IDLE_TIMER:
				#if 0//dennis
				cy_log_msg(CYLF_AUDIO, CY_LOG_WARNING, "%s(), atction:SLEEP_ACT_START_IDLE_TIMER\n", __func__);
				#endif
